...
- Measure heights at the **real serving widths**: the article column is
  ...
  is not monotonic in width. `scripts/verify/check_widget.py` sweeps correctly
  and also checks overflow and canvas aspect; `_static/viz-autosize.js`
  already sizes frames at runtime, so the `height=` attribute is only the
  no-JS fallback.
- I review on a phone. Zero horizontal overflow at 320 px is a requirement,
  not a nicety.

**Verification.** Decks and pages must be render-verified, not read. A page
that builds is not a page that renders. **The harness already exists** — do not
rebuild it:

```sh
pip install playwright && (cd scripts/verify && npm install)
TEXINPUTS=/workspace/latex-tools/tex/latex//: \
  scripts/verify/mech_check.sh <NN> <slug>
```

`scripts/verify/README.md` documents all five checkers. `mech_check.sh` runs
every mechanical check for one lesson — files present, LaTeX compiles, every
`\part` has a solution, deck and widgets render, LO markup matches the module,
and the grep-level gotchas. It was validated against all eight Module 2
lessons, so a failure on new work is a real defect rather than a harness bug.
Vendoring reveal.js and MathJax from npm is required because the CDNs are
blocked in these containers; the checkers stop with an error rather than
silently passing a deck with no math.
