...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\eq{ \Delta_{\text{min}} &= 8a = 3.2\ \text{mm} \\
		     N_{\text{max}} &= \frac{61.2\ \text{mm}}{3.2\ \text{mm}} = 19.1 \rightarrow N = 19 }
...
$Z_{\text{in}} = 86.9 + j46.2 \ohms$ and a peak gain of 2.17 dBi.
...
\begin{parts}
	...
	resonance, and directivity in dBi.
		...
		From the sinusoidal-current analysis: $Z_{\text{in}} \approx 73 + j42.5 \ohms$ at
		exactly $\lambda/2$; the antenna is resonant slightly short, at about
		$0.47$ to $0.48\ \lambda$; the model's own curve reads about $63 \ohms$
		at that crossing, against the $70 \ohms$ a bench measures;
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		values of $R_{\text{in}}$ and of $X_{\text{in}}$.
		\begin{solution}[1.3in]
		\eq{ \frac{86.9 - 73}{73} &= 0.190 \rightarrow 19.0\% \\
		     \frac{46.2 - 42.5}{42.5} &= 0.087 \rightarrow 8.7\% }
		The reactance agrees closely, while the resistance does not. Note also
		that the simulated gain of 2.17 dBi differs from the predicted 2.15 dBi
		by $0.02\db$ -- under half a percent in power -- so the pattern
		quantities agree even though the impedance does not.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$R_{\text{in}}$ error = \ansbox[1.2in]{$19.0\%$} \\[10pt]
		$X_{\text{in}}$ error = \ansbox[1.2in]{$8.7\%$}
	\end{minipage}

	\part In two or three sentences, account for the resistance difference. Name
	the mechanism; ``simulation error'' is not an explanation.
		\begin{solution}[1.3in]
		The value $73 + j42.5 \ohms$ is the impedance of the assumed sinusoidal
		current at exactly $\lambda/2$, not of the wire. The solved current
		differs from the sinusoid near the tips and at the feed, and the impedance
		is read at the feed, where that difference is largest, so the resistance
		comes out near $87 \ohms$. Both models put resonance near $0.47$ to
		$0.48\ \lambda$, so a wire cut to exactly $\lambda/2$ is about 5\% long
		under either one, which is why both call it inductive. The resistance gap
		comes from the difference in current shape, not from the length, and
		trimming does not remove it.
		\end{solution}

	\begin{minipage}[t]{0.58\linewidth}
		\part Trimming brings this wire to resonance at $0.473\ \lambda$. Find the
		resonant length in millimeters and the percent shortening from
		$\lambda/2$.
		\begin{solution}[1.2in]
		\eq{ L_{\text{res}} &= 0.473(122.4\ \text{mm}) = 57.9\ \text{mm} \\
	...
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$L_{\text{res}}$ = \ansbox[1.2in]{$57.9\ \text{mm}$} \\[10pt]
...
\begin{tabular}{c c c}
\hline
\textbf{Segments $N$} & \textbf{$R_{\text{in}}$ ($\Omega$)} & \textbf{$X_{\text{in}}$ ($\Omega$)} \\
\hline
5  & 79.9 & $+35.9$ \\
11 & 81.9 & $+43.9$ \\
21 & 83.4 & $+45.6$ \\
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part Compute the percent change in $R_{\text{in}}$ from each row to the next.
		\begin{solution}[1.2in]
		...
		The steps shrink, but slowly: $R_{\text{in}}$ is still creeping up by about a
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part Find the largest \emph{odd} segment count the $\Delta > 8a$ rule
		permits for this wire. Which rows of the table violate it?
		\begin{solution}[1.1in]
		\eq{ \Delta &= \frac{0.5\ \lambda}{N} > 8a = 0.008\ \lambda \\
		     N &< \frac{0.5}{0.008} = 62.5 \rightarrow N_{\text{max}} = 61 }
		The rule alone allows 62; 61 also keeps a segment at the feed. The
		$N = 81$ row breaks it --- its segments are only about five radii long ---
		so it sits outside the range where the thin-wire kernel is trustworthy
		and should not be used as evidence of anything.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$N_{\text{max}}$ = \ansbox[1.2in]{$61$}
	\end{minipage}
	...
	answer must reference both the convergence trend and the kernel limit.
		\begin{solution}[1.2in]
		41 segments. Each further refinement moves $R_{\text{in}}$ by about one
		percent, well below what you could measure on the bench, and
		$\Delta = 0.0122\ \lambda$ clears the $0.008\ \lambda$ floor with room for a
		shorter trimmed wire. Going to 81 buys a percent of drift at the cost of
		leaving the kernel's valid range.
		\end{solution}

	\part Explain in one or two sentences why gain converges at a lower segment
	count than input impedance does.
		\begin{solution}[0.9in]
		Gain is an integral over the entire current, so per-segment errors partly
		cancel and the total settles quickly. Input impedance is read from one
		segment --- the one carrying the source --- so it inherits that segment's
		full local error, which changes every time the segment length does.
		\end{solution}
...
\begin{parts}
	...
	lossless free-space model, and why is that a test rather than a result?
		...
		know the answer, so any deviation is a defect in the model rather than a
		physical result. It is also the cheapest error check available.
		\end{solution}
...
\begin{parts}
	...
	the radiation integral.
		...
		$E_{\text{tan}} = 0$ on the conductor, and solves a linear system for the segment
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		feed current of $8.90 - j4.80\ \text{mA}$. Find $Z_{\text{in}}$, then the magnitude
		of the reflection coefficient and the VSWR referred to $50 \ohms$.
		\begin{solution}[1.6in]
		\eq{ Z_{\text{in}} &= \frac{V}{I} = \frac{1}{\lp 8.90 - j4.80 \rp \ee{-3}} \\
		     &= \frac{\lp 8.90 + j4.80 \rp \ee{-3}}{1.0225 \ee{-4}} = 87.0 + j46.9 \ohms \\
		     \Gamma &= \frac{Z_{\text{in}} - 50}{Z_{\text{in}} + 50} = \frac{37.0 + j46.9}{137.0 + j46.9} \\
	...
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$Z_{\text{in}}$ = \ansbox[1.4in]{$87.0 + j46.9 \ohms$} \\[10pt]
	...
	sentences.
		\begin{solution}[1.1in]
		Trim the wire first. Trimming costs nothing, removes the $+j46.9 \ohms$ entirely,
