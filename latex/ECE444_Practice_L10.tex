...
%
% L10 practice set (Measurement Lab 1 - Impedance and S-parameters). Every
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		That is what the course bar, VSWR $\le 2$, looks like on a log-magnitude
		trace: this antenna sits exactly on it.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
	...
	\part Why is VSWR $\le 2$ the course's acceptance bar for an antenna match?
	Answer in terms of power, in one or two sentences.
		\begin{solution}[0.9in]
		At VSWR $= 2$, $|\Gamma| = 1/3$ and $|\Gamma|^2 = 1/9$: about $11\%$ of the
		delivered power comes back and $89\%$ goes into the antenna. Below that bar the
		reflected power is a small correction to the link budget, so a better match buys
		very little, while a worse one costs quickly. On a dB trace the bar is the
		$-9.5\db$ line; switch the analyzer to VSWR format and it is simply 2.
		\end{solution}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		dipole meet the VSWR $\le 2$ bar, and what one change fixes it?
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		VSWR $\le 2$ bar as measured?
		\begin{solution}[1.1in]
		\eq{
			|S_{11}| &= 20\ \mylog{0.33} = -9.6\db \\
			\text{VSWR} &= \frac{1.33}{0.67} = 1.99
		}
		Yes -- by $0.01$. It passes with no margin worth quoting.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
	...
	Compute it, and state what the result tells you about relying on trimming alone.
		...
		It passes, at VSWR $1.92$ -- but the perfect trim bought only $1.99 \to 1.92$.
		Trimming removes reactance; it cannot move the resistance, and here the
		resistance is what is left. An element whose radiation resistance sits far from
		$50$~\ohms needs a matching network or a different feed arrangement, not a
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part Interpolate the two VSWR $= 2$ crossings -- the $-9.5\db$ line on this
		trace -- and report the impedance bandwidth in MHz and as a percentage.
		\begin{solution}[1.5in]
		Lower crossing, between $880\ \mhz$ ($-8.8\db$) and $890\ \mhz$ ($-12.4\db$):
		\eq{
			f_1 = 880 + 10\ \frac{9.5-8.8}{12.4-8.8} = 882\ \mhz
		}
		Upper crossing, between $920\ \mhz$ ($-12.5\db$) and $930\ \mhz$ ($-9.0\db$):
		\eq{
			f_2 = 920 + 10\ \frac{12.5-9.5}{12.5-9.0} = 929\ \mhz
		}
		\eq{
			\text{BW} = 929 - 882 = 47\ \mhz, \qquad
			\frac{47}{905} = 5.2\%
		}
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		BW = \ansbox[1.2in]{$47\ \mhz$} \\[8pt]
		Fractional = \ansbox[1.2in]{$5.2\%$}
	\end{minipage}

	\begin{minipage}[t]{0.58\linewidth}
		\part At resonance the Smith-chart marker sits on the real axis, to the
		\emph{right} of center. Find $Z_{\text{in}}$ at resonance.
		\begin{solution}[1.2in]
		On the real axis $\Gamma$ is purely real, and right of center means it is
	...
	this element fit the rule of thumb, and what physical change would widen it?
		\begin{solution}[1.0in]
		Yes: $5.2\%$ sits in the lower half of the usual $3$--$10\%$ range, which is
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\textbf{Antenna B wins by about $2.4\db$} despite failing the match bar --
		$-9\db$ is VSWR $2.10$. Mismatch loss stays small well past VSWR $\le 2$;
		the bar is a design discipline, not a cliff.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
	...
	\part During the lab you bring a hand within a few centimeters of the element
	...
	you would check one of them.
		...
		against a standard-gain antenna on the range (Lesson 11): if the measured gain
