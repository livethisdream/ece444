...
$c = 3\times10^{8}\ \text{m/s}$ and $\eta = 377\ \Omega$.}
...
\begin{parts}
	...
	and say in one sentence what ``complementary'' means physically.
		\begin{solution}[1.1in]
		\eq{ Z_{\text{slot}}\ Z_{\text{dipole}} = \frac{\eta^{2}}{4} }
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\begin{solution}[1.1in]
		\eq{
		\frac{\eta^{2}}{4} &= \frac{(377)^{2}}{4} = 3.553\times10^{4}\ \Omega^{2} \\
...
$G = \varepsilon_{ap}\,4\pi A/\lambda^{2}$; take the optimum-horn value
$\varepsilon_{ap} = 0.5$ unless told otherwise.
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		to drop $\varepsilon_{ap}$ to $0.35$. What gain does it deliver, and how much was lost?
		\begin{solution}[1.1in]
		\eq{
	...
	eventually stops increasing its gain, and what the phrase ``optimum horn'' names.
		...
		$\varepsilon_{ap}$ falls; past a point the efficiency loses faster than the area gains and the
		...
		$3\lambda/8$ in the H-plane), which is where $\varepsilon_{ap} \approx 0.5$ comes from.
		\end{solution}
