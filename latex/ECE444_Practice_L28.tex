...
\begin{parts}
	...
	same weights. The sweep's noise floor sits about $22$\db below the uniform peak.
	Is the report credible? Explain.
		\begin{solution}[1.2in]
		No. The sweep cannot display anything below its own noise floor, so no point
		on the trace can read more than about $22$\db below the reference peak. A
	...
	\part The notch appears centred at $+19.7\mydeg$ rather than $+22.5\mydeg$.
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		reaches $20$ to $22$\db by the same argument.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
	...
	than holding a beam peak.
		...
		roughly $0.01$\db at the peak and sets a floor near $-21$\db in the null.
		\end{solution}
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part A requirement calls for a $33$\db notch. Taking $21$\db as the depth
		achievable with the 7-bit ADAR1000 and roughly $6$\db of improvement per
		added bit, how many phase bits are needed?
		\begin{solution}[1.1in]
		\eq{ 33\db - 21\db = 12\db \quad \Rightarrow \quad
		     \frac{12}{6} = 2 \ \text{additional bits} }
		\eq{ B = 7 + 2 = 9 \ \text{bits}, \qquad
		     \text{LSB} = \frac{360\mydeg}{512} = 0.70\mydeg }
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$B$ = \ansbox[1.3in]{$9\ \text{bits}$}
	\end{minipage}
