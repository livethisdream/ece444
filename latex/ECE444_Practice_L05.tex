...
infinitesimal dipole is a sum of three pieces, and $kr$ alone sets their
relative sizes:
\eq{ E_\theta \propto
...
\begin{parts}
	...
	\part We call the innermost region \emph{reactive}. Justify that word in one
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		Give it in wavelengths and in centimeters at $915\mhz$.
	...
	away. In two sentences, say what is wrong with the number he reports.
		...
		than the antenna. Gain only means something in the far field; there is no valid gain
...
from its center, by a path difference $\Delta \approx D^{2}/(8r)$. The agreed
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		A factor of $1600$ between two antennas sharing a frequency --- \emph{size}
		sets the far-field distance, not wavelength alone.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
