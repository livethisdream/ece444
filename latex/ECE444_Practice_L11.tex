...
Take the aperture efficiency to be $\varepsilon_{ap} = 0.65$ and
$c = 3\ee{8}\ \text{m/s}$.

\begin{parts}

	\part In one or two sentences, explain why the statement ``gain is really a
	...
	the antenna amplifies.''
		...
		small that solid angle can be. Writing $G = \varepsilon_{ap} 4\pi A /
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\eq{
			...
			G &= \varepsilon_{ap}\lp \frac{\pi D}{\lambda}\rp^{2}
			   = 0.65\,(60\pi)^{2} = 0.65\,(35531) \\
			  &= 2.31\ee{4}
			   \;\rightarrow\; 10\,\mylog{2.31\ee{4}} = 43.6\dbi
		}
		Remark: the effective aperture is $A_e = \varepsilon_{ap}A =
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\eq{
			...
			\varepsilon_{surf} &= 10^{-0.069/10} = 0.984 \\
			\varepsilon_{ap} &= (0.712)(0.984) = 0.700
		}
	...
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$\varepsilon_{ap}$ = \ansbox[1.3in]{$0.70$}
	\end{minipage}
	...
	proposal.
		...
		\eq{
			...
			\varepsilon_{surf} &= 10^{-0.171} = 0.674 \\
			\varepsilon_{ap} &= (0.712)(0.674) = 0.48
		}
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		parabolic dish that delivers it with $\varepsilon_{ap} = 0.60$.
		\begin{solution}[1.6in]
		$\lambda = 3\ee{8}/5.8\ee{9} = 0.0517\ \m$ and $G = 10^{25/10} = 316$.
		\eq{
			A_e &= \frac{G\lambda^{2}}{4\pi}
			     = \frac{316\,(0.0517)^{2}}{4\pi} = 0.0673\ \m^{2} \\
			A   &= \frac{A_e}{\varepsilon_{ap}} = \frac{0.0673}{0.60}
			     = 0.112\ \m^{2} \\
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		$\varepsilon_{ap} = 0.75$ and elements on a $\lambda/2$ grid. How many
	...
	you just computed.
		...
		loss at 5.8\ \ghz\ is real money and real dB. The array's advantages ---
		flat profile and the option of electronic steering --- are worth nothing
...
\begin{parts}
	...
	things that eat margin on a real rooftop link.
		\begin{solution}[1.3in]
		Deploy the dish link. A 2.3\db\ margin is not a link, it is a demo: the
		first bad day takes it down. Real margin eaters include rain and wet
		foliage in the path, mount drift and building sway pushing the beam off
