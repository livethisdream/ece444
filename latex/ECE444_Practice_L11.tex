...
%
% L11 practice set (Measurement Lab 2: Radiation Patterns). Every question is
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\question \textbf{Is the range adequate?} You are setting up a pattern range at
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		antenna the $2D^2/\lambda$ rule is often \emph{not} the one that
...
E-plane cut of an aperture antenna, recorded as $|S_{21}|$ in dB. The cut was
symmetric about $0\mydeg$ to within $0.3\db$, so only $\theta \ge 0$ is
tabulated. With the AUT unmated and its cable terminated in $50\ \ohms$, every
other setting unchanged, the same sweep read $-78.0\db$.
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\begin{solution}[1.3in]
		The peak is $-35.6\db$, so the half-power level is $-38.6\db$. That
		falls between $6\mydeg$ ($-38.3\db$) and $8\mydeg$ ($-41.4\db$):
		\eq{
...
Question 1 you record a peak of $-35.6\db$ with the AUT installed. You then
swap in the calibrated reference horn ($G_{ref} = 15.0\dbi$), change nothing
else, and record $-32.4\db$.
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		re-peak: the boresight level drops to $-59.9\db$. Compute the
...
pattern reaches its peak at $-35.6\db$; the terminated floor capture, taken at
the same settings, reads $-78.0\db$. Levels below are quoted relative to the peak.
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		The number is not usable. It is the difference of two nearly equal
...
aperture-formula prediction of $12.3\dbi$ (using $\eta_{\text{ap}} = 0.5$).
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		$20\ \mylog{0.023} = -32.6\db$ below the direct path. That is the purpose
		of the absorber on the bounce point.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
	...
	between your measurement and the prediction.
		...
		$\eta_{\text{ap}} = 0.5$, and real horns run anywhere from $0.4$ to
