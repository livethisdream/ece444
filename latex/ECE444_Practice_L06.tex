...
\begin{parts}
	...
	$R = |\vec{r} - \vec{r}^{\prime}|$. Write both and state where you use each.
		\begin{solution}[1.2in]
		Amplitude: $1/R \approx 1/r$ (a fractional error of order $r^{\prime}/r$).
		Phase: $R \approx r - \rhat \cdot \vec{r}^{\prime}$, because
		$k \rhat \cdot \vec{r}^{\prime}$ is several radians and controls whether
	...
	$\times$ (radiation vector), and say what each factor depends on.
		...
		\eq{ \vec{A} = \dfrac{\mu e^{-jkr}}{4\pi r}\vec{N}(\theta,\phi),
		     ...
		     \vec{N} = \int_{V^{\prime}} \vec{J}(\vec{r}^{\prime})
		               e^{+jk\rhat\cdot\vec{r}^{\prime}} dV^{\prime} }
	...
	the wire axis?
		...
		$\thetahat$ --- not part of the integral. At $\theta = 0\mydeg$ the
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\eq{ N_z(\theta) &= I_0 \int_{-L/2}^{L/2} e^{+jkz^{\prime}\cosp{\theta}} dz^{\prime} \\
		                 ...
		                 &= I_0 L \dfrac{\sinp{u}}{u},
		                  \quad u = \tfrac{kL}{2}\cosp{\theta} }
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$N_z$ = \ansbox[1.4in]{$I_0 L \sinp{u}/u$}
	\end{minipage}
	...
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		rule of thumb = \ansbox[1.2in]{$16.9\mydeg$}

		\vspace{4pt}
		exact = \ansbox[1.2in]{$17.0\mydeg$}
	\end{minipage}
	...
	relationship.
		...
		move}: it stays at $-13.3\db$, because the \emph{shape} sets it (the
...
$L = 4\lambda$ carries a progressive phase that moves the pattern peak off
broadside to $\theta_0 = 60\mydeg$. The excitation is
$I(z^{\prime}) = I_0 e^{-jkz^{\prime}\cosp{\theta_0}}$.
\begin{parts}

	\begin{minipage}[t]{0.58\linewidth}
		\part Show from the radiation integral that the peak lands at $\theta_0$, and
		state the peak angle for this design.
		...
		= 60\mydeg$. A linear phase across the source shifts the transform; it does not
		reshape it.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$\theta_{\text{peak}}$ = \ansbox[1.3in]{$\theta_0 = 60\mydeg$}
	\end{minipage}
...
\begin{parts}
	...
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$|F(90\mydeg)|$ = \ansbox[1.2in]{$1$ (peak)}

		\vspace{4pt}
		$|F(0\mydeg)|$ = \ansbox[1.2in]{$0$ (null)}
	\end{minipage}
	...
	$\vec{J}$ hard to know, and name the numerical method that solves for it.
		\begin{solution}[1.1in]
		The fields set the current, and the current sets the fields --- a
...
$-26.5\db$, and $-31.5\db$. Work these parts from the \textbf{taper table in
Lesson 6}, which pairs each distribution with its first-sidelobe level and also
records the price: relative to uniform illumination, a taper broadens the main
beam by roughly $1.3\times$ to $1.6\times$.
\begin{parts}
	...
	\part All four have the same $L$. Rank their half-power beamwidths, put an
	approximate broadening factor on each, and explain the trade.
		\begin{solution}[1.5in]
		Uniform is \emph{narrowest} ($1.00\times$, by definition), then cosine
		($\approx 1.34\times$), then triangular ($\approx 1.43\times$), then
		cosine-squared (widest, $\approx 1.63\times$). The ranking is the sidelobe
		ranking read backwards: tapering suppresses the outer part of the aperture, so
		the \emph{effective} length shrinks and the beam broadens. Same $L$ does
		\textbf{not} mean same beamwidth. \textbf{Sidelobe level is bought with
		beamwidth} --- the central trade of aperture design (L15, L24).
		\end{solution}

	\begin{minipage}[t]{0.58\linewidth}
		\part Your requirement is a $2\mydeg$ beam with sidelobes below $-25\db$.
		Which knob sets which requirement? Size the aperture, in wavelengths, both
		before and after you apply the taper you chose.
		\begin{solution}[2.0in]
		\textbf{Length} sets the beamwidth, \textbf{taper shape} sets the sidelobe
		level. Start with uniform illumination and
		$\theta_{\text{HP}} \approx 0.886\lambda/L$:
		\eq{
			\theta_{\text{HP}} &= 2\mydeg = 2\lp \dfrac{\pi}{180} \rp = 0.0349 \text{ rad} \\
			L &= \dfrac{0.886\lambda}{0.0349} \approx 25.4\lambda
		}
		Uniform gives only $-13.3\db$ sidelobes, so pick the cheapest taper that clears
		$-25\db$: \textbf{triangular}, at $-26.5\db$. Its beamwidth constant is
		$1.27\lambda/L$ ($\approx 1.43\times$ uniform), so grow the aperture by the same factor to hold $2\mydeg$:
		\eq{ L &= \dfrac{1.27\lambda}{0.0349} \approx 1.43 \lp 25.4\lambda \rp \approx 36.4\lambda }
		Cosine-squared would also clear the spec but costs $1.63\times$, i.e.\
		$\approx 41\lambda$ --- more aperture than the requirement is asking you to buy.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$L$ (uniform) = \ansbox[1.2in]{$25.4\lambda$}

		\vspace{4pt}
		$L$ (tapered) = \ansbox[1.2in]{$36.4\lambda$}
	\end{minipage}
