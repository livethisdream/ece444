...
% deliberately absent; they belong to L12.
...
\begin{parts}
	...
	two sentences.
		...
		--- it is a donut, not a beam. Directivity integrates over the whole sphere, and
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\begin{solution}[1.4in]
		\eq{
		\Gamma &= \frac{Z_{\text{in}} - Z_0}{Z_{\text{in}} + Z_0} = \frac{23 + j42.5}{123 + j42.5} \\
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part Compute the reactance of a half-wave dipole from $X = \frac{\eta_0}{4\pi}Si(2\pi)$,
		and state in one sentence why the wire radius does not appear in the answer.
		\begin{solution}[1.4in]
		\eq{
		\frac{\eta_0}{4\pi} &= \frac{377}{4\pi} = 30.0\ohms \\
