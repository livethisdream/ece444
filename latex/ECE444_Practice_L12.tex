...
%
% L12 practice set (Loop and Monopole Antennas) -- image theory, the quarter-wave
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		The whip is about 62 cm of metal above the plane.
		\end{solution}
	\end{minipage}\hfill%
...
\begin{parts}
	...
	deck radiates essentially nothing.
		...
		with them, so nearly all of the input power is dissipated rather than radiated.
		\end{solution}
...
\question \textbf{The full loss budget of a magnetic loop.} A single-turn circular loop of radius
...
$R_{\text{ohmic}} = \lp C/2\pi b \rp R_s$ for a single turn.
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\eq{ R_{\text{ohmic}} &= \frac{C}{2\pi b} R_s = \frac{a}{b} R_s \\
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\eq{ R_r &= 20^2 \lp 0.0125 \rp = 5.00\ \ohms \\
		     R_{\text{ohmic}} &= 20 \lp 0.098 \rp = 1.96\ \ohms \\
...
$Q \gtrsim 1/(ka)^3 + 1/(ka)$. Lesson 3's fractional bandwidth is roughly $1/Q$ at
the half-power points, or $0.71/Q$ at the course's VSWR $\le 2$ bar.
\begin{parts}

	\begin{minipage}[t]{0.58\linewidth}
		\part Compute $ka$, the Chu $Q$, and the resulting VSWR $\le 2$ bandwidth
		in kHz.
		\begin{solution}[1.4in]
		\eq{ k &= \frac{2\pi}{21.1} = 0.297\ \text{rad/m}, \quad ka = 0.0892 \\
		     Q &\gtrsim \frac{1}{(0.0892)^3} + \frac{1}{0.0892} = 1409 + 11 = 1420 \\
		     \text{FBW} &\approx \frac{0.71}{Q} = 5.0\ee{-4} = 0.050\% \\
		     \Delta f &= 0.00050 \lp 14.2\ \mhz \rp = 7.1\ \khz }
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
	...
	does your answer imply about operating across a 350 kHz amateur band?
		\begin{solution}[1.2in]
		Yes. 7.1 kHz covers one 3 kHz voice channel with room to spare, so the loop is usable on
		any single frequency. Moving elsewhere in a 350 kHz band means retuning the loop capacitor,
		because the match degrades within a channel or two. The size reduction is paid for in
		retuning.
		\end{solution}
	...
	than your Chu estimate. Why? Is that good news?
		...
		roughly in proportion to the efficiency, $Q_{\text{loaded}} \approx \eta_{\text{rad}} Q \approx
		0.113(1420) \approx 160$, which widens the band to $0.71/160 = 0.44\%$, or 63 kHz --
		nearly nine times the Chu estimate. It is
		not good news: the extra bandwidth is bought by dissipating power in copper. A
		$50\ \ohms$ resistor is matched at every frequency and radiates nothing at all.
		\end{solution}
