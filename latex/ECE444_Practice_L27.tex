...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		This RMS estimate is consistent with the course number: evaluating the rounded
		weights numerically leaves a residual of about $-48$\db at the designed angle.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
	...
	$2\db$ below that same reference.
		\begin{solution}[1.3in]
		The weights are good to about $-48\db$ (the RMS estimate above reads $-46\db$),
		so quantization is not what the plot is
