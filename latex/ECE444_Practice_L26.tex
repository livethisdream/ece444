...
is fed, so the fed elements sit $42\ \text{mm}$ apart. Work at the array design centre,
...
\begin{parts}
	...
	the main lobe, while a sidelobe does not.
		...
		At a grating-lobe angle the path-length difference between neighbouring elements is
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		largest usable element spacing in millimetres.
		\begin{solution}[1.3in]
...
for $\theta_0 = 45\mydeg$ at the band centre, $f_0 = 10.25\ \ghz$. Use
...
\begin{parts}
	...
	\part A pattern null on the PHASER can be driven only about $21\ \db$ deep. Explain in
	one or two sentences why the phase LSB sets that limit.
		\begin{solution}[1.2in]
		A null is produced by arranging element contributions that cancel, and the depth of
		the cancellation is set by how exactly the residual can be driven to zero. With a
		$2.8125\mydeg$ phase step the residual phase error can be up to half an LSB per
		element, so a small uncancelled vector always remains. That leftover sets a floor on
		the notch depth, measured at about $21\ \db$ on the course hardware once the roughly
		1\% gain steps are included.
		\end{solution}
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		$(0.024)\tanp{50\mydeg} = 0.029\ \text{rad} = 1.6\mydeg$ either side of centre. That
