...
%
% L13 practice set (Patch, Slot, and Horn Antennas) -- sizing a rectangular
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part Find the patch width $W$ in millimeters.
...
\begin{parts}
	...
	slot here rather than a monopole.
		...
		vertical dipole. The slot is chosen because it is flush: it has no protrusion, adds no drag, and has nothing to
...
$G = \eta_{\text{ap}}\,4\pi A/\lambda^{2}$; take the optimum-horn value
$\eta_{\text{ap}} = 0.5$ unless told otherwise.
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		to drop $\eta_{\text{ap}}$ to $0.35$. What gain does it deliver, and how much was lost?
		\begin{solution}[1.1in]
		\eq{
	...
	eventually stops increasing its gain, and what the phrase ``optimum horn'' names.
		...
		$\eta_{\text{ap}}$ falls; past a point the efficiency loses faster than the area gains and the
		...
		$3\lambda/8$ in the H-plane), which is where $\eta_{\text{ap}} \approx 0.5$ comes from.
		\end{solution}
...
\begin{parts}
	...
	\part The two radiating slots sit only about a quarter of a free-space wavelength apart. Say
	...
	near $6$\dbi{} rather than up with the horns.
		...
		so a patch radiates a broad hemispherical beam worth roughly $5$ to $8$\dbi. Horns get their
	...
	the conflict this creates with the patch element and one concrete way to resolve it.
		...
		thickness, layers, or both; the bandwidth has to come from somewhere.
		\end{solution}
