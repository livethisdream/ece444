...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part Lesson 7 predicted $Z_{in} = 73 + j42.5$~\ohms for an ideal half-wave
		...
		so the reactance goes to zero and $R_{in}$ falls to roughly $70$~\ohms:
		\eq{
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part Write $\Gamma$ in rectangular form and find the input impedance $Z_{in}$.
		...
		\eq{
			Z_{in} &= 50\ \frac{0.891 + j0.279}{1.707} \\
	...
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$Z_{in}$ = \ansbox[1.5in]{$26.1 + j8.2$~\ohms}
	\end{minipage}
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\emph{right} of centre. Find $Z_{in}$ at resonance.
		\begin{solution}[1.2in]
		On the real axis $\Gamma$ is purely real, and right of centre means it is
		positive, so $R_{in} > 50$~\ohms:
		\eq{
			|\Gamma| &= 10^{-19.4/20} = 0.107 \\
			Z_{in} &= 50\ \frac{1+0.107}{1-0.107} = 50 \lp 1.240 \rp = 62\ \ohms
		}
...
\begin{parts}
	...
	and, separately, to the measured impedance. Why is this combination dangerous?
		...
		including the sign of the reactance. That is dangerous precisely because the
		plot that students trust is the one that still looks fine.
		\end{solution}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		Nearly a full rotation of the chart.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		other frequency it is rotated. Getting the right answer at one frequency is
		luck, not calibration -- fix it with port extension or by calibrating at the
		antenna connector.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\textbf{Antenna B wins by about $2.4\db$}, despite the far uglier $S_{11}$.
	...
	to the depth of the dip, and why neither effect is instrument error.
		...
		the antenna's $Q$. The input impedance genuinely changed, so the VNA is
		reporting the truth -- the hand became part of the antenna.
		\end{solution}
	...
	you would check one of them.
		...
		accepts, and looks beautifully matched while doing it. (2) \textbf{Pattern or
