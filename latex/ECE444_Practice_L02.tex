...
\begin{parts}
	...
	any twice-differentiable $f$, and find $u$ in terms of $L$ and $C$.
		...
		\eq{ f^{\prime\prime}(\xi) = LC\,u^{2} f^{\prime\prime}(\xi) \;\Longrightarrow\; u = \dfrac{1}{\sqrt{LC}}. }
		\end{solution}
	...
	RG-58 coax) means physically.
		...
		Signals travel at about 66\% of the speed of light in vacuum --- the dielectric
		between the conductors slows the wave, so the guided wavelength on the line is
		$0.66\times$ the free-space wavelength at the same frequency.
		\end{solution}
...
\begin{parts}
	...
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$\omega$ = \ansbox[1.3in]{$1.51\times10^{10}$}
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\eq{ D &\approx \dfrac{41{,}253}{(2.5)(1.8)} = \dfrac{41{,}253}{4.5} \approx 9170 \\
			D_\mathrm{dBi} &= 10\,\mylog{9170} \approx 39.6\dbi }
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part The antenna is measured to have radiation efficiency $88\%$ and impedance
		mismatch $\left|\Gamma\right| = 0.25$. What is the realized gain at boresight, in dBi?
		\begin{solution}[1.6in]
		\eq{ G &= \eta_\mathrm{rad}\,D = 0.88(9170) \approx 8070 \;\to\; 39.1\dbi \\
			1 - \left|\Gamma\right|^{2} &= 1 - 0.0625 = 0.9375 \\
			G_\mathrm{re} &= 0.9375(8070) \approx 7560 \;\to\; 38.8\dbi }
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$G_\mathrm{re}$ = \ansbox[1.3in]{$\approx 38.8\dbi$}
	\end{minipage}

	\part In one sentence, explain the difference between $D$, $G$, and
	$G_\mathrm{re}$ for this antenna.
		\begin{solution}[1.1in]
		$D$ is purely geometric (pattern shape only); $G$ folds in the ohmic and
		dielectric losses inside the antenna; $G_\mathrm{re}$ additionally subtracts the
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		efficiency $\eta_\mathrm{ap} = A_{e}/A_\mathrm{phys}$? Is that reasonable?
		\begin{solution}[1.4in]
		\eq{ A_\mathrm{phys} &= \pi(1.2)^{2} \approx 4.52\ \text{m}^{2} \\
			\eta_\mathrm{ap} &= \dfrac{2.49}{4.52} \approx 0.55 }
		Realistic --- real dishes fall in $\eta_\mathrm{ap} \approx 0.5$--$0.7$ once
		illumination taper, spillover, feed/strut blockage, and surface roughness are counted.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$\eta_\mathrm{ap}$ = \ansbox[1.3in]{$\approx 0.55$}
	\end{minipage}

	\begin{minipage}[t]{0.58\linewidth}
		\part Keeping the same physical dish but re-illuminating it efficiently at
		$f = 30\ghz$ (Ka-band), what gain (dBi) would you expect if $\eta_\mathrm{ap}$
		is unchanged?
		\begin{solution}[1.4in]
		...
		(Optimistic: higher $f$ exposes surface roughness that erodes $\eta_\mathrm{ap}$.)
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
...
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\question \textbf{Reading a pattern.} The polar plot handed out in class (or the
one embedded in the lesson deck) shows the E-plane cut of a horn antenna. From
the plot, estimate each quantity and record your reading. \emph{Answers are
reading-dependent; the tolerances below are the grading rubric.}
\begin{parts}
	\part Boresight direction (degrees off nose).
		\begin{solution}[0.5in] Graded within $\pm2\mydeg$ of the true peak. \end{solution}
	\part Half-power beamwidth (HPBW).
		\begin{solution}[0.5in] Graded within $\pm10\%$ of the plotted value. \end{solution}
	\part First-null beamwidth (FNBW), or state if no null is visible in range.
		\begin{solution}[0.5in] Within $\pm10\%$; ``no null visible'' is legitimate if the cut is monotonic to the plot edge. \end{solution}
	\part Peak sidelobe level (SLL), in dB below the main lobe.
		\begin{solution}[0.5in] Within $\pm2\db$ of the plotted peak sidelobe. \end{solution}
	\part Front-to-back ratio (F/B), in dB.
		\begin{solution}[0.6in] Within $\pm2\db$. The habit being built is reading pattern plots \emph{consistently} --- every later lesson reuses these same measurements. \end{solution}
\end{parts}
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part Compute the on-axis power density $S_\mathrm{inc}$ at the cubesat.
		\begin{solution}[1.5in]
		$G_{t} = 8\dbi \to 6.31$.
		\eq{ S_\mathrm{inc} &= \dfrac{P_{t} G_{t}}{4\pi r^{2}} = \dfrac{20(6.31)}{4\pi(8.0\times10^{5})^{2}} \\
			&\approx 1.57\times10^{-11}\ \text{W/m}^{2} }
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$S_\mathrm{inc}$ = \ansbox[1.3in]{$1.57\times10^{-11}$}
	\end{minipage}
	...
	\begin{minipage}[t]{0.58\linewidth}
		\part Compute the received power $P_{r} = S_\mathrm{inc}\,A_{e,r}$ and express
		the result in dBm.
		\begin{solution}[1.3in]
		\eq{ P_{r} &= (1.57\times10^{-11})(0.0895) \approx 1.40\times10^{-12}\watts \\
			P_{r,\mathrm{dBm}} &= 10\,\mylog{\dfrac{1.40\times10^{-12}}{10^{-3}}} \approx -88.5\dbm }
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
	...
	\part Verify your answer by computing $P_{r}$ directly from the Friis equation
	$P_{r} = P_{t}\,G_{t}\,G_{r}\lp \dfrac{\lambda}{4\pi r} \rp^{2}$.
		\begin{solution}[1.6in]
		\eq{ \lp \dfrac{\lambda}{4\pi r} \rp^{2} &= \lp \dfrac{0.75}{4\pi(8.0\times10^{5})} \rp^{2} \approx 5.56\times10^{-15} \\
			P_{r} &= 20(6.31)(2.00)(5.56\times10^{-15}) \approx 1.40\times10^{-12}\watts \;\checkmark }
		Same answer --- Friis is just $S_\mathrm{inc}\times A_{e,r}$ with $A_{e,r}$
		written out in terms of $G_{r}$ and $\lambda$. Same physics.
		\end{solution}
