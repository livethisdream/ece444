...
%
% L9 practice set (Pattern Measurement Theory) -- range length from the pi/8
...
obviously; it returns a pattern that still looks like a pattern.
\begin{parts}
	...
	\emph{square} of the distance from the aperture center.
		\begin{solution}[1.2in]
		The incident wavefront is a sphere of radius $r$ centered on the source,
		not a plane. A point a distance $x$ off the aperture center is
...
\begin{parts}
	...
	the critical ones.
		\begin{solution}[1.2in]
		The pyramids are a gradual impedance taper: at normal incidence a wave
		travels far down into the lossy foam before it can reflect, so it is
		absorbed. At grazing incidence it sees the tips as a nearly continuous
		surface, travels only a short distance in the lossy material, and
		reflects far more strongly. The critical surfaces are therefore the
		\textbf{side walls, ceiling, and floor} in the specular region between
		source and antenna, where stray rays strike at a slant.
		\end{solution}
	...
	sentence, and say what happens to the measurement if you proceed anyway.
		...
		The antenna does not fit the volume the chamber is certified for: its
		outer $0.15\ \m$ on each side sees fields with no reflectivity guarantee
		at all. Those edges are what set the sidelobe structure, so any sidelobe
		or null from the run is indefensible -- even though the main beam and
		gain will still look fine.
		\end{solution}
...
\question \textbf{Two ways to an absolute number.} Pattern shape needs only a
normalization; gain needs a calibration.
\begin{parts}
	...
	Explain in one or two sentences why they do not matter.
		\begin{solution}[1.1in]
		Write Friis for each measurement. Every one of those quantities appears
		identically in both, because nothing but the antenna on the positioner
		changed, so all of them cancel in the difference $P_{AUT} - P_{SGH}$. The
		method measures a \emph{ratio} of gains; the horn's calibration supplies
		the only absolute number in the room.
		\end{solution}
	...
	result.
		\begin{solution}[1.5in]
		VSWR $= 2.0$ gives $\vert \Gamma \vert = 1/3$, so the fraction of incident
		power accepted is $1 - \vert \Gamma \vert ^2 = 0.889$:
		\eq{ \text{mismatch loss} = -10\ \mylog{0.889} = 0.51\db }
		The antenna under test accepts $0.51\db$ less than a matched antenna of
		the same gain, so the comparison reads $0.51\db$ \textbf{low}:
		$31.8\dbi$ measured against $32.3\dbi$ true. Report it as \emph{realized
		gain} -- the system-level number -- and quote $32.3\dbi$ separately if
		the customer wants gain proper.
		\end{solution}
...
\begin{parts}
	...
	not do.
		...
		The near-field to far-field step is a Fourier transform, which operates on
		the complex field. Magnitude alone does not determine it: infinitely many
		phase distributions share one magnitude on the scan plane and give
		entirely different far fields. Hence a phase-coherent receiver and
		phase-stable cabling.
		\end{solution}

	\part Explain, in two or three sentences, why the near-field to far-field
	transform is exact rather than a curve fit. Name the earlier result it
	rests on.
		\begin{solution}[1.2in]
		L6 established that the far field is the Fourier transform of the source
		distribution, with $k\sinp{\theta}$ as the spatial frequency variable.
		Transforms are invertible, so the complex tangential field on a surface
		enclosing the antenna fixes the field everywhere outside it, uniquely and
		with no fitting. It is that same relationship run backwards.
		\end{solution}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		At a few points per second, several hours of scanning -- the cost of a
		near-field range.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
	...
	and name one thing that set of sweeps still will not tell you.
		...
		configurations: E-plane co-pol (source aligned with the horn's E-field)
		and E-plane cross-pol (source rotated $90\mydeg$), then H-plane co-pol
		and H-plane cross-pol. That is \textbf{four sweeps}. What it misses is
		the diagonal ($45\mydeg$) planes, where a rectangular aperture's highest
		sidelobes usually sit; those need a conical cut or a full spherical scan.
		\end{solution}
