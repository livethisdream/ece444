...
\begin{parts}
	...
	or two sentences why it is not the dipole value.
		...
		$Z_{in} = 36.5 + j21.3\ \ohms$, exactly half the dipole value. The image in the ground
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\eq{ \Gamma &= \frac{Z_{in} - Z_0}{Z_{in} + Z_0}
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\eq{ R_{ohmic} &= \frac{C}{2\pi b} R_s = \frac{a}{b} R_s \\
		     ...
		     \eta_{cd} &= \frac{R_r}{R_r + R_{ohmic}} = \frac{0.0125}{0.1105} = 0.113 \\
...
\begin{parts}
	...
	\begin{minipage}[t]{0.58\linewidth}
		...
		\eq{ R_r &= 20^2 \lp 0.0125 \rp = 5.00\ \ohms \\
		     ...
		     \eta_{cd} &= \frac{5.00}{6.96} = 0.718 }
		71.8\%, a loss of only $1.4\db$: gain $= 1.76 - 1.4 = 0.4\dbi$.
		\end{solution}
	\end{minipage}\hfill%
	\begin{minipage}[t]{0.37\linewidth}
		\raggedleft \vspace{12pt}
		$\eta_{cd}$ = \ansbox[1.3in]{$71.8\%$}
	\end{minipage}
...
$a = 0.30\ \m$ at $14.2\ \mhz$ ($\eta_{cd} = 0.113$). The Chu limit from Lesson 3 gives the
...
\begin{parts}
	...
	than your Chu estimate. Why? Is that good news?
		...
		roughly in proportion to the efficiency, $Q_{loaded} \approx \eta_{cd} Q \approx
