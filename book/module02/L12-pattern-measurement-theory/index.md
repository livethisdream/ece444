...
<h1 class="frame-title">Pattern Measurement Theory</h1>
...
<ol class="lo-list lo-sublist" style="--module: '2'; --lo: '5'; counter-reset: lo 3">
...
::::{frame} Worked example — how long a range does a 0.5 m antenna need?
...
::::{frame} Worked example — how long a range does a 0.5 m antenna need? (cont.)
:::{admonition} Worked example — how long a range does a 0.5 m antenna need? (cont.)
:class: tip
...
::::{frame} A small-antenna exception
```{note}
...
::::{frame} What a short range does to your data
...
::::{frame} The nulls fill long before the main beam moves
:class: viz-frame
...
::::{frame} What the top two rows say
...
::::{frame} The fourth changes the question
| Range type | How it makes the plane wave | What limits it |
| :-- | :-- | :-- |
...
::::{frame} Phase is not optional
A transform needs the complex field. Magnitude alone does not determine it, which is why a near-field range is built around a vector network analyzer and phase-stable cables, and why the probe position has to be known to a small fraction of a wavelength.
::::

::::{frame} Sample at half-wavelength spacing or finer
This is the same Nyquist argument as in L6's spatial spectrum: undersample the aperture field and the transform aliases, producing lobes in the far-field pattern that the antenna does not have.
::::

::::{frame} Scan far enough out
The transform assumes the whole field was captured. Truncate the scan plane before the field has decayed and you get truncation error, which is why planar scanning gives a trustworthy pattern only over a forward cone — roughly $\pm 60^\circ$ — and says nothing about the back lobes.
::::
...
::::{frame} Why the comparison method works
...
::::{frame} Worked example — comparison-method gain
...
::::{frame} Conditions for a valid comparison
...
::::{frame} Worked example — two-antenna method
...
::::{frame} Worked example — two-antenna method (cont.)
:::{admonition} Worked example — two-antenna method (cont.)
:class: tip
...
::::{frame} The three-antenna method
...
::::{frame} Co-polarization and cross-polarization
...
::::{frame} The spinning-linear method
...
::::{frame} Summary — compact range and near-field scanning
| Symbol / idea | What it is | The number to remember |
| :-- | :-- | :-- |
