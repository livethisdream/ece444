...
$$G = \varepsilon_{ap} \frac{4\pi A}{\lambda^2} \quad\quad A_e = \varepsilon_{ap} A = \frac{G \lambda^2}{4\pi}$$

- $A/\lambda^2$ counts **square wavelengths**, not square metres
- An aperture is never simply *big* — it is big **at a frequency**
- Circular dish of diameter $D$: $\quad G = \varepsilon_{ap} \left( \pi D / \lambda \right)^2$
- Good reflectors: $\varepsilon_{ap} \approx 0.55$ to $0.7$; horns $\approx 0.5$
...
| $D/\lambda$ | Gain at $\varepsilon_{ap}=0.55$ | HPBW |
| :-- | :-- | :-- |
...
Home satellite-TV dish, Ku band, $\varepsilon_{ap} = 0.65$.
...
**Product: 0.66.** That is where $\varepsilon_{ap} \approx 0.55$ to $0.7$ comes from — four unavoidable trades, not sloppiness.
