...
async function api(path, body) {
  // Two hosts, one app. The local tool answers over HTTP; the hosted build
  // answers in the page, where nec_lab's Python runs under Pyodide against a
  // WebAssembly NEC. Same endpoints, same shapes -- the difference stops here.
  if (window.NEC_LAB_LOCAL) return window.NEC_LAB_LOCAL(path, body || {});
...
// Through api(), not fetch: the hosted build has no server to GET from.
api('engine').then((info) => {
