...
**Nulls.** $\sin\pi u$ vanishes at $u = \pm 1, \pm 2, \pm 3, \dots$, so the first null sits at $\sin\theta = \lambda/L$. An aperture shorter than one wavelength has no null anywhere in real space, which is why a small aperture radiates broadly no matter how it is fed.

**Beamwidth.** Solving $\sin(\pi u)/(\pi u) = 1/\sqrt{2}$ gives $\pi u = 1.3916$, so the half-power points sit at $u = \pm 0.4429$ and
