...
## Part 1: Where the phase ramp comes from
...
$n\ d\sin\theta_0$ shorter — it sees the wave that much sooner. Divide by the
speed of light and the geometry becomes timing:

$$\Delta t = \frac{d\sin\theta_0}{c}.$$

For the course array steered to $30^\circ$, that is $7\ \text{mm}$ of path and
about $23\ \text{ps}$ of arrival-time difference per element. Now convert the
delay of a sinusoid into a phase. A signal delayed by $\Delta t$ at frequency
$f$ is retarded in phase by $\omega\Delta t$, so the element-to-element phase
difference produced by the incoming wave is
...
## Part 2: Setting the eight phases
...
## Part 3: Predicting the steered pattern
...
lobe points at the commanded angle. Every other feature of the pattern moves
with it, but *rigidly in $\sin\theta$, not in $\theta$*. Plot the pattern
against $\sin\theta$ and steering slides the whole curve sideways without
changing its shape. Plot it against $\theta$ and the same curve stretches as it
moves, which is exactly the beam broadening of Part 4.
...
## Part 5: Reading the phases backwards
