...
window.RevealChalkboard = window.RevealChalkboard || {
	...
	colorIndex: function ( idx ) {
		/* DFEC PATCH: upstream drops the argument, so any API caller set
		   color[mode] = undefined and stroke colors only changed by luck.
		   Forward it. Re-apply if the plugin is upgraded. */
		colorIndex( idx );
	},
	colorNext: function () {
		colorNext();
 ...
 ******************************************************************/
  ...
  var sponge = 		{
		/* DFEC PATCH: upstream hardcodes img/sponge.png here, bypassing the
		   eraser config entirely — the file is not vendored and 404s on every
		   eraser selection. Crosshair matches the theme's pen cursors.
		   Re-apply if the plugin is upgraded. */
		cursor: 'crosshair'
	}
...
//console.log("Add drawing", newPDFPage);
	...
	function eraseWithSponge( context, x, y ) {
		/* DFEC PATCH: the upstream circle is centered at (x+radius, y+radius)
		   because the stock sponge IMAGE rides the cursor with its top-left
		   corner at the pointer. Our theme ships an invisible 1x1 eraser
		   cursor, so the un-centered circle made users aim above-left of the
		   target. Centered on the pointer instead; the recorded event x/y is
		   unchanged, and the theme's print replay also centers, so live and
		   printed erases agree. Re-apply if the plugin is upgraded. */
		context.save();
		context.beginPath();
